...
| 9 | 🟡 | infra | `criar-pipeline-de-deploy-no-github-actions` | 5, 7 | Workflow manual: build→ECR→deploy via SSM Run Command. **Código pronto + `plan` OK (role OIDC, 4 add); falta `apply` da role + config dos GitHub Environments + 1º dispatch.** |

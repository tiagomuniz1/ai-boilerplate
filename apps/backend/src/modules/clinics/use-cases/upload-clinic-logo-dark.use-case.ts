...
export class UploadClinicLogoDarkUseCase extends BaseUseCase {
  ...
  async execute(clinicId: string, file: Express.Multer.File): Promise<ClinicResponseDto> {
    ...
    const logoDarkPath = await this.storageAdapter.upload(file.buffer, path, file.mimetype)

    await this.clinicsRepository.updateLogoDark(clinicId, logoDarkPath)
    ...
    return this.clinicResponseMapper.toResponse({ ...clinic, logoDarkPath, updatedAt: new Date() })
  }
}

...
@Injectable()
export class CompleteAppointmentUseCase extends BaseUseCase {
  private readonly logger = new Logger(CompleteAppointmentUseCase.name)
  ...
  async execute(id: string, currentUser: ICurrentUser): Promise<AppointmentResponseDto> {
    ...
    if (appointment.date > today) {
      throw new UnprocessableEntityException('Cannot complete a future appointment')
    }

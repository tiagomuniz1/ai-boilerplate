...
import { COUNCIL_TYPE_LABELS, ExamRequestSnapshot } from '@app/shared'
...
@Injectable()
export class ExamRequestPdfBuilderService implements OnModuleInit {
  onModuleInit() {
  ...
  private buildDocDefinition(snapshot: ExamRequestSnapshot, logoBase64: string | null) {
    ...
    content.push({ text: 'SOLICITAÇÃO DE EXAMES', style: 'title', alignment: 'center', margin: [0, 20, 0, 16] })
  ...
  private buildHeader(snapshot: ExamRequestSnapshot, logoBase64: string | null): object[] {
  ...
  private buildFooter(snapshot: ExamRequestSnapshot): object[] {

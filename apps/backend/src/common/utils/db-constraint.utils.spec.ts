...
describe('isUniqueConstraintViolation', () => {
  it('returns true when error is QueryFailedError with code 23505 and matching constraint', () => {
    const error = makeQueryError('23505', DB_UNIQUE_CONSTRAINTS.USERS_EMAIL_CLINIC)
    expect(isUniqueConstraintViolation(error, DB_UNIQUE_CONSTRAINTS.USERS_EMAIL_CLINIC)).toBe(true)
  })

  it('returns false when error is not a QueryFailedError', () => {
    expect(isUniqueConstraintViolation(new Error('generic'), DB_UNIQUE_CONSTRAINTS.USERS_EMAIL_CLINIC)).toBe(false)
  })

  it('returns false when error code is not 23505', () => {
    const error = makeQueryError('23502', DB_UNIQUE_CONSTRAINTS.USERS_EMAIL_CLINIC)
    expect(isUniqueConstraintViolation(error, DB_UNIQUE_CONSTRAINTS.USERS_EMAIL_CLINIC)).toBe(false)
  })
  ...
  it('returns false for null', () => {
    expect(isUniqueConstraintViolation(null, DB_UNIQUE_CONSTRAINTS.USERS_EMAIL_CLINIC)).toBe(false)
  })
})

...
import { useCreateClinicAdminUser } from './use-create-clinic-admin-user.hook'
...
describe('useCreateClinicAdminUser', () => {
  const input = { fullName: 'Alice', email: 'alice@example.com', password: 'password123', role: UserRole.ADMIN }
  const model = { id: 'uuid-1', fullName: 'Alice', email: 'alice@example.com', role: UserRole.ADMIN, createdAt: new Date(), updatedAt: new Date() }
  ...
  it('calls createUserUseCase with input merged with clinicId', async () => {
    ...
    await waitFor(() => {
      ...
      expect(firstArg).toEqual({ ...input, clinicId: CLINIC_ID })
    })
  })
  ...
  it('does not navigate on error', async () => {
    const error = { status: 422, title: 'Unprocessable Entity', detail: 'Clinic not found' }

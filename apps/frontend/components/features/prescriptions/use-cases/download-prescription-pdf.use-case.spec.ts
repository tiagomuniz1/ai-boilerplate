...
import { downloadPrescriptionPdfUseCase } from './download-prescription-pdf.use-case'
...
describe('downloadPrescriptionPdfUseCase', () => {
  ...
  it('downloads PDF with default filename', async () => {
    ...
    await downloadPrescriptionPdfUseCase('rx-uuid')

    expect(mockService.downloadPdf).toHaveBeenCalledWith('rx-uuid')
    expect(mockCreateObjectURL).toHaveBeenCalledWith(blob)
    expect(anchorElement.href).toBe(mockObjectUrl)
    expect(anchorElement.download).toBe('receita-rx-uuid.pdf')
  ...
  it('downloads PDF with custom filename', async () => {
    ...
    await downloadPrescriptionPdfUseCase('rx-uuid', 'my-prescription.pdf')

    expect(anchorElement.download).toBe('my-prescription.pdf')
  })

  it('propagates service errors', async () => {
    mockService.downloadPdf.mockRejectedValue({ status: 403 })
    await expect(downloadPrescriptionPdfUseCase('rx-uuid')).rejects.toMatchObject({ status: 403 })
